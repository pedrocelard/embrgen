...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.dl = cycle(data.DataLoader(self.ds, batch_size = train_batch_size, shuffle=False, pin_memory=False))
    ...
    def load(self, milestone, **kwargs):
        ...
        data = torch.load(str(self.results_folder / f'uncentered_model-{milestone}.pt'))
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step % self.update_ema_every == 0:
                self.step_ema()
                f = open('log.txt', 'w')
                f.write('Working on stem - %s' % self.step)
                f.close()
